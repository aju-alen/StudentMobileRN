...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return definePageSeo({
    title:
      locale === 'ar'
        ? 'المدونة لعائلات الإمارات | كوتش أكاديم'
        : 'Blog for UAE Families | Coach Academ',
    description: copy.resources.blogLead,
    primaryKeywords:
      locale === 'ar'
        ? ['الدروس الخصوصية الإمارات', 'كيف تجد معلماً في دبي']
        : ['tutoring UAE', 'how to find a tutor Dubai'],
      ...
      <div className="home-section-inner max-w-3xl">
        <header className="mb-10 sm:mb-12">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold text-gray-900 mb-4 leading-tight">
            {copy.resources.blogTitle}
          </h1>
          <p className="text-lg sm:text-xl text-gray-600 leading-relaxed">
            {copy.resources.blogLead}
          </p>
        </header>
        ...
        <ul className="space-y-6">
          {articles.map((article) => (
            <li key={article.slug}>
              <Link
                href={withLocale(`/blog/${article.slug}`, locale)}
